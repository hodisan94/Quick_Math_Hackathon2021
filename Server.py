...
class Server:
    def __init__(self, tcp_port, port_ip):
        self.udp_port = 13117
        ...
        self.tcp_socket.bind((port_ip, self.tcp_port))
    ...
    def broadcast(self):
        ...
        :return:
        '''
        print("Server started, listening on IP address " + str(scapy.get_if_addr("eth1")))
        while not self.check_players():
            self.broad_udp_socket.sendto(self.msg, (
            '.'.join(scapy.get_if_addr("eth1").split('.')[:2]) + ".255.255", self.udp_port))
            time.sleep(1)
    ...
    def start_game(self):
        ...
        t_for_player1 = Thread(target=self.get_answer, args=[self.player_client1, pause_event, who_won_who_won,
                                                             1])  ### need to figure out if the list really changed
        t_for_player2 = Thread(target=self.get_answer, args=[self.player_client2, pause_event, who_won_who_won,
                                                             2])  ### need to figure out if the list really changed
